<template>
    <div v-if="workflow.visualization"
         class="chart-builder-options">
        <b-form class="chart-properties">
            <b-card sub-title="Opções da visualização">
                <b-form-group id="title"
                              label="Tipo do gráfico:"
                              label-for="title">
                    <v-select v-model="workflow.visualization.forms.type.value"
                              :options="chartTypes"
                              label="label"
                              :reduce="(opt) => opt.name">
            ...
            <b-card sub-title="Eixo X"
                    class="mt-1">
                <b-form-group label="Atributo:">
                    <v-select v-model="workflow.group.forms.attributes.value"
                              :options="attributes"
                              label="name"
                              :reduce="(opt) => opt.name"
                              :multiple="true">
            ...
            <b-card sub-title="Eixo Y"
                    class="mt-1"
                    style="z-index:10">
                <b-form-group label="Atributo(s):">
                    <v-select ref="vueSelect"
                              v-model="workflow.group.forms.function.value"
                              :options="attributes"
                              label="name"
                              :reduce="reduceYAxis"
                              :multiple="multipleY">
                        ...
                        <template #selected-option="{name, type, f}">
                            <div role="button"
                                 ...
                                 @mousedown.stop="handleSelect(name)"
                                 @mouseup.stop="handleSelect(name)">
                                <column-aggregate v-if="name"
                                                  :name="name"
                                                  :type="type"
                                                  :f="f"
            ...
            <b-card sub-title="Formatação"
                    class="mt-1"
                    style="z-index:1">
                <b-form-group>
                    <b-form-checkbox v-model="workflow.visualization.forms.display_legend.value"
                                     switch>
                        Exibir legenda
                    </b-form-checkbox>
                </b-form-group>
                ...
                <b-form-group>
                    <ColorPalette :field="palette"
                                  :value="workflow.visualization.forms.palette.value"
                ...
                <template v-if="workflow.visualization.forms.type.value=='bar'">
                    ...
                    <b-form-group>
                        <b-form-checkbox v-model="workflow.visualization.forms.stacked.value"
                                         switch>
                            Empilhado
                        </b-form-checkbox>
                    </b-form-group>
                ...
                <template v-if="workflow.visualization.forms.type.value=='pie'">
                    <b-form-group label="Preenchimento (100 = pizza, < 100 donut)"
                                  label-for="pie-fill">
                        <b-form-input id="title"
                                      v-model="workflow.visualization.forms.hole.value"
                                      type="number"
                                      min="0"
                                      max="100"
                                      step="1"
                ...
                <template v-if="workflow.visualization.forms.type.value=='line'">
                    <b-form-group label="Espessura da Linha:"
                        ...
                        <b-form-input v-model="workflow.visualization.forms.line_stroke.value"
                                      type="number"
                                      min="1"
                                      max="10"
                                      step="1"
                    ...
                    <b-form-group label="Modo para as séries:">
                        <select v-model="workflow.visualization.forms.mode.value"
                                class="form-control form-control-sm">
                            <option value="lines">
                                Somente linhas
                            </option>
                ...
                <template v-if="workflow.visualization.forms.type.value=='bubble'">
                    <b-form-group label="Símbolo"
                                  label-for="line-width">
                        <b-form-select v-model="chartCustomOptions.bubbleChart.symbol"
                                       :options="[
