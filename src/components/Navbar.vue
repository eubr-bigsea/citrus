<template>
    <b-navbar id="l-navbar" toggleable="md" type="dark" variant="dark">
        ...
        <b-nav-item-dropdown v-if="getUserIsAdmin()" :text="$tc('titles.administration', 2)" right>
          <b-dropdown-item :to="{ name: 'AdministrationUserList' }">{{ $tc('titles.user', 2) }}</b-dropdown-item>
        ...
        <b-nav-item-dropdown v-if="getUserIsAdmin()" :text="$tc('titles.tool', 2)" right>
          <b-dropdown-item href="#">{{$tc('atmosphere.transparency', 2)}}</b-dropdown-item>
        </b-nav-item-dropdown>
      </b-navbar-nav>
