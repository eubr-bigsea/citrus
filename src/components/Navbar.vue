    ...
    <b-navbar id="l-navbar" toggleable="md" type="dark" variant="dark">
        ...
        <b-collapse id="nav_collapse" is-nav>
            <b-navbar-nav>
                ...
                <b-nav-item-dropdown v-if="hasRoles" :text="$tc('titles.administration', 2)" right>
...
<script>
    ...
    export default {
        ...
        methods: {
            getUserFirstName() {
                let aux = this.$store.getters.user;
                const user = aux.attributes || aux;
                return (user.name || user.first_name).split(' ')[0];
            },
            getUserAvatar() {
