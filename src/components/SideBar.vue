        ...
        <ul id="menu" class="sidebar-nav nav-pills nav-stacked">
            ...
            <li :class="{ active: path === '/projects' }">
                <b-link v-if="hasAnyPermission(DATA_SOURCE_PERMISSIONS) || isAdmin" :to="{ name: 'projects' }"
                    ...
                    <font-awesome-icon icon="fa fa-shapes" size="lg" fixed-width /> {{ $t('titles.project', 2) }}
                </b-link>
            </li>
            <li :class="{ active: path === '/data-sources' }">
                <b-link v-if="hasAnyPermission(DATA_SOURCE_PERMISSIONS) || isAdmin" :to="{ name: 'dataSources' }"
                    class="text-success-2">
                    <font-awesome-icon icon="fa fa-database" size="lg" fixed-width /> {{ $t('titles.dataSource', 2) }}
                </b-link>
            </li>
            <li :class="{ active: path === '/workflows' }">
                <b-link v-if="hasAnyPermission(WORKFLOW_PERMISSIONS) || isAdmin" :to="{ name: 'workflows' }"
                    class="text-success-2">
                    <font-awesome-icon icon="fa fa-project-diagram" size="lg" fixed-width /> {{ $t('titles.workflow',
                        2) }}
                </b-link>
            </li>
            <li :class="{ active: path === '/experiments' }">
                <b-link v-if="hasAnyPermission(APP_PERMISSIONS) || isAdmin" :to="{ name: 'index-explorer' }"
                    class="text-success-2">
                    <font-awesome-icon icon="fa fa-vial" size="lg" fixed-width /> {{ $t('titles.dataExplorer', 2) }}
                </b-link>
            </li>
            <li :class="{ active: path === '/apps' }">
                <b-link v-if="hasAnyPermission(APP_PERMISSIONS) || isAdmin" :to="{ name: 'tracks' }"
                    class="text-success-2">
                    <font-awesome-icon icon="fa fa-microscope" size="lg" fixed-width /> {{ $t('titles.track', 2) }}
                </b-link>
            </li>
            <li :class="{ active: path === '/jobs' }">
                <b-link v-if="hasAnyPermission(JOB_PERMISSIONS) || isAdmin" :to="{ name: 'jobs' }"
                    class="text-success-2">
                    <font-awesome-icon icon="fa fa-tasks" size="lg" fixed-width /> {{ $t('titles.jobs', 2) }}
                </b-link>
            </li>
            <li :class="{ active: path === '/dashboards' }">
                <b-link v-if="hasAnyPermission(DASHBOARD_PERMISSIONS) || isAdmin" :to="{ name: 'dashboards' }"
                    class="text-success-2">
                    <font-awesome-icon icon="fa fa-chart-line" size="lg" fixed-width /> {{ $t('titles.dashboard', 2) }}
                </b-link>
            </li>
            <li class="mt-5" />

            <li :class="{ active: path === '/admin' }">
                <b-link v-if="isAdmin" :to="{ name: 'admin-start' }" class="text-success-2">
                    <font-awesome-icon icon="fa fa-user-lock" size="lg" fixed-width /> {{ $t('titles.administration',
                        2) }}
                </b-link>
            </li>
...
export default {
    ...
    methods: {
        toggle() {
            // Verifica se a ref 'wrapper' existe antes de tentar acessar
            if (this.$refs.wrapper) {
                this.$refs.wrapper.classList.toggle("toggled-2");
