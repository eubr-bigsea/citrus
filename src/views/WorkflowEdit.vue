    ...
    <div class="row">
      <div class="col">
        <div>
          ...
          <b-tabs
            @input="updateSelectedTab"
            ref="formTabs"
            v-model="selectedTab"
            nav-class="custom-tab"
          >
            <b-tab
              v-for="form of workflow.platform.forms"
              :title-item-class="'tab-order-' + form.order"
                ...
                <div class="row">
                  ...
                  >
                    ...
                    <div slot="modal-footer" class="w-100 text-right">
                      ...
                      <button
                        class="ml-1 btn btn-sm btn-outline-dark"
                        @click="cancelExecute"
                      ...
                      <div class="row">
                        <div class="col-md-12 mb-3">
                          <b-form-radio
                            ...
                            v-model="saveOption"
                            value="new"
                          >{{$t('workflow.newName')}}</b-form-radio>
                          <input
                            type="text"
                            maxlength="40"
                            class="form-control"
          ...
          <b-modal id="taskResultModal" ref="taskResultModal" :title="resultTask.name">
            ...
            <div v-for="log in resultTask.step.logs" :key="log.id">{{log}}</div>
            <div>{{resultTask.result}}</div>
          </b-modal>

          <b-modal
            id="validationErrorsModal"
            size="lg"
          ...
          <b-modal
            id="workflowProperties"
            size="lg"
            ref="workflowProperties"
            :title="$tc('titles.property', 2)"
            :okOnly="true"
          >
            <b-form @submit="saveWorkflowProperties" v-if="loaded">
              <b-form-group :label="$tc('common.name', 1) + ':'">
                <b-form-input id="exampleInput1" type="text" v-model="workflow.name" required></b-form-input>
              </b-form-group>
              ...
              <b-form-checkbox v-model="workflow.is_template">
                ...
                <br>
                <small>
                  <em>{{$t('workflow.useAsTemplateExplanation')}}</em>
                </small>
              </b-form-checkbox>
            </b-form>
          </b-modal>
...
export default {
  ...
  mounted() {
    ...
    this.$root.$on(
      'update-workflow-form-field-value',
      (field, value, labelValue) => {
        const self = this;
        if (self.workflow)
          if (
            !self.workflow.forms ||
            !(self.workflow.forms instanceof Object)
          ) {
            self.workflow.forms = {};
          }
        try {
    ...
    this.$root.$on('addFlow', (flow, jsPlumbConn) => {
      flow.id = `${flow.source_id}/${flow.source_port}-${flow.target_id}/${
        flow.target_port
      }`;
    ...
    this.$root.$on('removeFlow', flowId => {
      const inx = this.workflow.flows.findIndex(n => {
        const id = `${n.source_id}/${n.source_port}-${n.target_id}/${
          n.target_port
        }`;
        return id === flowId;
      });
  ...
  methods: {
    ...
    toggleTasks(mode, prop) {
      this.$refs.diagram.toggleTasks(mode, prop);
    },
    distribute(mode, prop) {
      this.$refs.diagram.distribute(mode, prop);
    },
    ...
    load() {
      let self = this;
      axios
        .get(`${tahitiUrl}/workflows/${this.$route.params.id}`)
        .then(resp => {
          ...
          axios
            ...
            .then(resp => {
              ...
              if (usingDisabledOp) {
                self.warning(
                  self.$t('messages.usingDisabledOperation', {
              ...
              axios
                .get(`${standUrl}/jobs/latest`, { params })
                .then(resp2 => {
    ...
    saveAsImage() {
      ...
      html2canvas($elem, {
        ...
        for (let flow of $elem.getElementsByClassName('jtk-connector')) {
          ...
          const xml = `<svg width="${flow.width.baseVal.value}" height="${
            flow.height.baseVal.value
          }" xmlns="http://www.w3.org/2000/svg">${flow.innerHTML}</svg>`;
        ...
        window.setTimeout(() => {
          ...
          targetCtx.fillText(
            `${self.workflow.name}. ${self.$t(
              'workflow.imageGeneratedAt'
            )} ${new Date()}`,
            20,
    ...
    saveWorkflow(savingCopy, newName) {
      ...
      if (savingCopy) {
        ...
        cloned.flows.forEach(flow => {
          ...
          flow.id = `${newSource}/${flow.source_port}-${newTarget}/${
            flow.target_port
          }`;
        });
      }

      return axios[method](url, cloned, { headers })
        .then(resp => {
          if (!savingCopy) {
            ...
            self.workflow = workflow;
            self.success(
              self.$t('messages.savedWithSuccess', {
                what: self.$tc('titles.workflow')
              })
            );
            self.isDirty = false;
            ...
            self.success(
              self.$t('workflow.copySavedWithSuccess', {
                what: self.$tc('titles.workflow')
              })
            );
          }
          ...
          self.$refs.diagram.clearWorkflow().then(() => {
            ...
            axios
              ...
              .then(resp => {
                ...
                self.success(
                  self.$t('workflow.versionRestored', {
                    version,
                    version2: 2343
                  })
                );
        ...
        if (this.attributeSuggestion[taskId]) {
          return this._unique(
            Array.prototype.concat.apply(
              [],
              this.attributeSuggestion[taskId].inputs.map(item => {
                return item.attributes;
              })
            )
          ).sort(this._caseInsensitiveComparator);
        } else {
          return [];
        }
      }
    ...
    updateAttributeSuggestion(callback) {
      ...
      try {
        window.TahitiAttributeSuggester.compute(
          self.workflow,
    ...
    showPropertiesWindow() {
      if (this.$refs.workflowProperties) this.$refs.workflowProperties.show();
    },
    showSaveAs() {
              ...
              form.fields.forEach(field => {
                if (field.enabled || field.enabled === undefined) {
                  if (field.required && field.enable_conditions !== 'false') {
                    const value = t.forms[field.name]
                      ? t.forms[field.name].value
                      : null;
                    if (
                      value === null ||
                      value === '' ||
      ...
      if (window.TahitiAttributeSuggester.cached[id]) {
        ...
        axios.get(url).then(
          response => {
            ...
            attributes = ds.attributes.map(function(attr) {
              return attr.name;
            });
