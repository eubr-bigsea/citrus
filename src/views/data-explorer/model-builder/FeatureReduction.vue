    ...
    <div>
        ...
        <div class="row">
            <div class="col-4">
                ...
                <template v-if="copy.forms.method.value === 'pca'">
                    <label>Número de componentes principais (k):</label>
                    <input v-model.number="copy.forms.k.value" type="number" class="form-control form-control-sm w-25"
                           maxlength="4" step="1" min="2">
                </template>
            </div>
            <div class="col-8">
                <div v-if="copy.forms.method.value !== 'disabled' && selected !== null">
...
export default {
    ...
    computed: {
        selected() {
            return this.options.find((opt) => opt.value === this.copy.forms.method.value);
        }
    },
    watch: {
        copy: {
